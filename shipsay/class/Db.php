...
class Db
{
	...
	public function __construct($dbarr)
	{
		...
		$this->conn=mysqli_connect($tmpstr.Page::Rico($dbarr['host']).':'.$dbarr['port'],$dbarr['user'],$dbarr['pass'],$dbarr['name'])or print('数据库连接失败');
	...
	{
		global $is_acode,$is_ft,$use_orderid,$sys_ver,$is_keywords;
		$res=mysqli_query($this->conn,$sql);
		if($res->num_rows)
		{
			$k=0;
			while($row=mysqli_fetch_assoc($res))
			{
				...
				if($is_keywords&&empty($row['keywords']))
				{
					global $keywords_num;
					require_once 'Keywords.php';
					$row['keywords']=Analysis::getKeywords($ret_arr[$k]['articlename'].$ret_arr[$k]['author'].$ret_arr[$k]['intro_des'],$keywords_num);
					$this->ss_query('UPDATE '.$this->dbarr['pre'].'article_article SET keywords = "'.$row['keywords'].'" WHERE articleid = '.$row['articleid']);
				}
