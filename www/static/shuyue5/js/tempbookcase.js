...
function removebook(k) {
    layer.confirm('确定要从书架删除本书吗？', {btn: ['删除', '取消']}, function (index) {
        lastread.remove(k);
        layer.close(index);
...
function removeall() {
    layer.confirm('确定要清空临时书架吗？', {btn: ['清空', '取消']}, function (index) {
        lastread.removeAll();
        layer.close(index);
        window.location.reload();
    }, function (index) {
...
function showtempbooks() {
    ...
    if (books.length) {
        for (var i = 0; i < books.length; i++) {
            if (i < bookmax) {
                bookhtml += '<tr><td class="hidden-xs">' + books[i][5] + '</td><td><a href="' + books[i][0] + '" target="_blank">' + books[i][2] + '</a></td><td><a href="' + books[i][1] + '">' + books[i][3] + '</a></td><td class="hidden-xs">' + books[i][4] + '</td><td class="delbutton"><a class="del_but" href="javascript:removebook(\'' + books[i][0] + '\')">删除</a></td></tr>'
            }
        }
