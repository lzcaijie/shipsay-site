...
<header class="topbar">
  <div class="wrap">
    ...
    <form class="search" action="<?=function_exists('ss_search_url')?ss_search_url():(isset($fake_search)?$fake_search:'/search/')?>" method="get">
      <input type="text" name="searchkey" placeholder="书名 / 作者" autocomplete="off">
      <button type="submit">搜索</button>
    </form>
    <a class="topbtn" href="<?=function_exists('ss_recentread_url')?ss_recentread_url():(isset($fake_recentread)?$fake_recentread:'/history.html')?>">记录</a>
  </div>
</header>
