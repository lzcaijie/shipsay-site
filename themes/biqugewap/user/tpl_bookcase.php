...
<body>
	...
	<div class="rank mt0 mb0 min-height">
		<h4>会员书架<a href="/logout/" onclick="return confirm('确定要退出吗？')" class="pull-right">退出登录</a></h4>
		<div class="content">
			...
			<dl>
				...
				<dd class="history"><?php if($v['chaptername'] !='0'): ?><a href="<?=$v['case_url']?>" target="_blank"><?=$v['chaptername']?></a><?php else:?>还没有读过本书！<?php endif ?></dd>
				<dd class="history"><?=$v['lastupdate_cn']?>更新：<a href="<?=$v['last_url']?>"><?=$v['lastchapter']?></a></dd>
				<dd><a href="<?=$v['author_url']?>"><?=$v['author']?></a><a href="javascript:;" onclick="delbookcase('<?=$v['articleid']?>');" id="del_temp">移除</a></dd>
			</dl>
				 <?php endforeach ?>
		</div>
		<div class="clear"></div>
