...
<div class="container" style="max-width:1200px;margin:0 auto;padding:12px;">
  ...
  <div style="margin:10px 0;">
    <?php
      $rank_base = isset($rank_detail_base) && $rank_detail_base ? rtrim((string)$rank_detail_base, '/') . '/' : ('/' . ((isset($fake_rankstr) && $fake_rankstr) ? trim($fake_rankstr, '/') : 'rank') . '/');
      $top_entry = isset($rank_entry_url) && $rank_entry_url ? rtrim((string)$rank_entry_url, '/') . '/' : ((isset($fake_top) && $fake_top) ? rtrim((string)$fake_top, '/') . '/' : $rank_base);
    ?>
