...
<head>
  ...
  $current_title = isset($title_arr[$current_query]) ? (string)$title_arr[$current_query] : '排行榜';
  ...
  <style>
    form.search{display:flex;align-items:center;gap:8px;flex-wrap:nowrap;box-sizing:border-box;}
    form.search .searchinput{flex:1 1 auto;min-width:0;box-sizing:border-box;}
    form.search .go{flex:0 0 auto;white-space:nowrap;padding:0 12px;}
    .rank-switch-list{display:flex;flex-wrap:wrap;margin:0 -4px;padding:0;}
    ...
    .rank-switch-list a.active strong{color:#fff;}
    .rank-list-items li{white-space:normal;overflow:hidden;text-overflow:ellipsis;}
    .rank-list-items li a{color:#208181;}
    @media (max-width:480px){
      .header h1{width:42%;font-size:18px;}
      .reg{font-size:14px;}
      .login_topbtn{padding:4px 6px;margin-left:3px;}
      form.search{gap:6px;padding:10px;}
      form.search .searchinput{font-size:14px;padding:0 8px;}
      form.search .go{padding:0 10px;min-width:56px;}
      .rank-switch-list li{width:50%;}
      ...
      <div class="block">
        <ul class="rank-switch-list">
          ...
          <?php foreach ($title_arr as $k => $t): ?>
            ...
            <li>
              <a href="<?=$tab_url_attr?>"<?=$current_query === $k ? ' class="active"' : ''?>><?php if ($current_query === $k): ?><strong><?=htmlspecialchars((string)$t, ENT_QUOTES, 'UTF-8')?></strong><?php else: ?><?=htmlspecialchars((string)$t, ENT_QUOTES, 'UTF-8')?><?php endif; ?></a>
            </li>
          <?php endforeach; ?>
        </ul>
      </div>
