...
$last_rows = (!empty($lastarr) && is_array($lastarr)) ? $lastarr : [];
if (empty($last_rows) && !empty($lastchapter_arr) && is_array($lastchapter_arr)) $last_rows = array_slice($lastchapter_arr, 0, 12);
$preview_rows = [];
if (!empty($chapterrows) && is_array($chapterrows)) $preview_rows = array_slice($chapterrows, 0, 50);
elseif (!empty($preview_chapters) && is_array($preview_chapters)) $preview_rows = array_slice($preview_chapters, 0, 50);
...
<div class="bookinfo ss-book-panel">
...
<div class="lb_mulu ss-chapter-block"><div class="book-itemtitle">最新章节</div><div class="cc"></div><ul class="last9 latest-list"><?php foreach($last_rows as $k => $v): ?><li class="<?php if($k % 2 != 0):?>even<?php endif?>"><a href="<?=htmlspecialchars((string)$v['cid_url'], ENT_QUOTES, 'UTF-8')?>"><?=htmlspecialchars((string)$v['cname'], ENT_QUOTES, 'UTF-8')?></a></li><?php endforeach ?></ul></div>
<?php endif; ?>
<?php if (!empty($preview_rows)): ?>
<div class="lb_mulu ss-chapter-block ss-indexlist-wrap"><div class="book-itemtitle">前50章</div><div class="cc"></div><ul class="last9 chapter-preview-list"><?php foreach($preview_rows as $k => $v): ?><li class="<?php if($k % 2 != 0):?>even<?php endif?>"><a href="<?=htmlspecialchars((string)$v['cid_url'], ENT_QUOTES, 'UTF-8')?>"><?=htmlspecialchars((string)$v['cname'], ENT_QUOTES, 'UTF-8')?></a></li><?php endforeach ?><?php if ($index_url_raw !== ''): ?><li class="more"><a href="<?=$index_url_attr?>">查看全部目录&gt;&gt;</a></li><?php endif; ?></ul></div>
<?php endif; ?>
<?php if ($is_langtail == 1 && !empty($langtailrows_safe)) : ?><div class="s_m related-box"><div class="q_top c_big"><p class="c_big_border">相关推荐</p></div><div class="cc"></div><div class="related-links"><?php foreach ($langtailrows_safe as $v) : ?><a href="<?=htmlspecialchars((string)$v['info_url'], ENT_QUOTES, 'UTF-8')?>"><?=htmlspecialchars((string)$v['langname'], ENT_QUOTES, 'UTF-8')?></a><?php endforeach ?></div></div><?php endif; ?>
