...
foreach (['Baiduspider','bingbot','360Spider','Sogou web spider','YisouSpider'] as $bot) {
...
<meta property="og:title" content="<?=htmlspecialchars((string)$sortname . '小说《' . (string)$articlename . '》最新章节《' . (string)$chaptername . '》,作者' . (string)$author, ENT_QUOTES, 'UTF-8')?>">
<meta property="og:description" content="<?=htmlspecialchars((string)$sortname . '小说《' . (string)$articlename . '》的最新章节', ENT_QUOTES, 'UTF-8')?>">
<style>
...
<?php
$home_url_attr = ss_h((string)ss_home_url());
$recentread_url_raw = (string)ss_recentread_url();
$recentread_url_attr = ss_h($recentread_url_raw);
ss_render_page_top(['page_title' => (string)$articlename, 'page_back_url' => $info_url_raw, 'show_back' => true]);
?>
<div class="reader-shell">
    ...
    <div class="reader-main">
        ...
        <h1 class="nr_title" id="nr_title">
            <?=htmlspecialchars((string)$chaptername, ENT_QUOTES, 'UTF-8')?>
            <?php if ($max_pid > 1): ?>
                <span style="font-size:14px;color:#666;margin-left:10px;">（第<?=$now_pid?>页/共<?=$max_pid?>页）</span>
            <?php endif; ?>
        </h1>
        ...
        <div class="nr_page">
            <table cellpadding="0" cellspacing="0">
                <tr>
                    <td class="prev"><a id="pt_index" href="<?=$home_url_attr?>">首页</a></td>
                    <td class="mulu">
                        <?php if ($recentread_url_raw !== ''): ?>
                            <a id="pt_bookcase" href="<?=$recentread_url_attr?>" rel="nofollow">阅读记录</a>
                        <?php else: ?>
                            <a id="pt_bookcase" href="javascript:void(0);" class="w_gray">阅读记录</a>
                        <?php endif; ?>
                    </td>
                    <td class="mulu"><a id="shuqian" href="<?=$info_url_attr?>">书籍详情</a></td>
                    <td class="next">
                        <?php if ($index_url_raw !== ''): ?>
                            <a id="pt_mulu" href="<?=$index_url_attr?>">返回目录</a>
                        <?php else: ?>
                            <a id="pt_mulu" href="javascript:void(0);" class="w_gray">返回目录</a>
                        <?php endif; ?>
                    </td>
                </tr>
            </table>
        ...
        <div class="nr_page">
            <table cellpadding="0" cellspacing="0" style="margin:5px 0;">
                <tr>
                    <td class="prev">
                        <?php if ($prevpage_url != ''): ?>
                            <a id="pb_prev" href="<?=htmlspecialchars((string)$prevpage_url, ENT_QUOTES, 'UTF-8')?>">上一页</a>
                        <?php else: ?>
                            <?php if ($pre_cid == 0): ?>
                                <a id="pb_prev" href="javascript:void(0);" class="w_gray">书首页</a>
                            <?php else: ?>
                                <a id="pb_prev" href="<?=htmlspecialchars((string)$pre_url, ENT_QUOTES, 'UTF-8')?>">上一章</a>
                            <?php endif; ?>
                        <?php endif; ?>
                    </td>
                    <td class="mulu">
                        <?php if ($index_url_raw !== ''): ?>
                            <a id="pb_mulu" href="<?=$index_url_attr?>">目录</a>
                        <?php else: ?>
                            <a id="pb_mulu" href="javascript:void(0);" class="w_gray">目录</a>
                        <?php endif; ?>
                    </td>
                    <td class="next">
                        <?php if ($nextpage_url != ''): ?>
                            <a id="pb_next" href="<?=htmlspecialchars((string)$nextpage_url, ENT_QUOTES, 'UTF-8')?>">下一页</a>
                        <?php else: ?>
                            <?php if ($next_cid == 0): ?>
                                <a id="pb_next" href="javascript:void(0);" class="w_gray">书末页</a>
                            <?php else: ?>
                                <a id="pb_next" href="<?=htmlspecialchars((string)$next_url, ENT_QUOTES, 'UTF-8')?>">下一章</a>
                            <?php endif; ?>
                        <?php endif; ?>
                    </td>
                </tr>
...
setTimeout(function(){
    $.ajax({
        ...
        data:{articleid:'<?=addslashes((string)$articleid)?>',chapterid:'<?=addslashes((string)$chapterid)?>',pid:'<?=addslashes((string)$now_pid)?>'},
        success:function(data){ $('#nr1').html(data); },
        error:function(){ $('#nr1').html('<div class="error-text">加载失败，请刷新重试</div>'); }
    });
}, 200);
<?php endif; ?>
</script>
