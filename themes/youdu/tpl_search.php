...
$search_key_safe = htmlspecialchars(isset($searchkey) ? (string)$searchkey : '', ENT_QUOTES, 'UTF-8');
$search_count_safe = isset($search_count) ? (int)$search_count : 0;
$search_res_safe = (!empty($search_res) && is_array($search_res)) ? $search_res : [];
$search_url_safe = '';
    ...
    <div class="ser-wrap">
        <form id="searchForm" name="t_frmsearch"<?php if ($search_url_safe !== ''): ?> action="<?=$search_url_safe?>" method="post"<?php else: ?> action="javascript:;" onsubmit="return false;"<?php endif; ?> class="ser-form mb10 pr">
            <input
                ...
                id="search"
                autocomplete="off"
                value="<?=$search_key_safe?>"
                <?php if ($search_url_safe === ''): ?>disabled="disabled" aria-disabled="true"<?php endif; ?>
            >
            <input type="hidden" name="searchtype" value="all">
            <label for="submit" class="pa t50p l0 _i">
                <svg><use xlink:href="#i-search"></use></svg>
                <button id="submit" type="submit" class="hide"<?php if ($search_url_safe === ''): ?> disabled="disabled" aria-disabled="true"<?php endif; ?>>submit</button>
            </label>
        </form>
        <?php if ($search_key_safe !== ''): ?><p class="fs16 c_small mb20">搜索“<?=$search_key_safe?>”找到 <?=$search_count_safe?> 条结果</p><?php endif; ?>
    </div>
        ...
        <div class="j_result_wrap">
            <div class="j_list_container" id="imgload">
                <ul class="ser-ret lh1d5">
                    <?php if ($search_count_safe > 0 && !empty($search_res_safe)): ?>
                        <?php foreach($search_res_safe as $k => $v): ?>
                            <?php
                            ...
                            <li class="pr pb20 mb20">
                                <a href="<?=$info_url?>" class="g_thumb pa l0 oh" title="<?=htmlspecialchars(isset($v['articlename']) ? $v['articlename'] : '', ENT_QUOTES, 'UTF-8')?>">
                                    <img src="<?=$nopic?>"<?php if (!empty($img_url)): ?> _src="<?=$img_url?>"<?php endif; ?> width="140" height="186" alt="<?=htmlspecialchars(isset($v['articlename']) ? $v['articlename'] : '', ENT_QUOTES, 'UTF-8')?>" onerror="this.onerror=null;this.src='<?=$nopic?>';">
                                </a>
                                <h3 class="mb5 fs20 f_mbo pt5 ell">
                                    <a href="<?=$info_url?>" class="c_strong" title="<?=htmlspecialchars(isset($v['articlename']) ? $v['articlename'] : '', ENT_QUOTES, 'UTF-8')?>"><?=isset($v['articlename']) ? $v['articlename'] : ''?></a>
                                </h3>
