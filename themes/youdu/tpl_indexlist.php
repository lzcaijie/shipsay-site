...
<div class="g_wrap pb30 pt10"><section class="youdu-section"><div class="youdu-section-head"><h3>最新12章</h3><span class="meta"><?php if($lastchapter_safe !== ''): ?>最新：<a href="<?=$esc($last_url_safe)?>"><?=$esc($lastchapter_safe)?></a><?php endif; ?><?php if($lastupdate_text !== ''): ?>&nbsp;&nbsp;<?=$esc($lastupdate_text)?><?php endif; ?></span></div><div class="youdu-chapter-grid"><?php foreach($latest_rows_safe as $row): ?><a href="<?=$esc($row['cid_url'])?>" title="<?=$esc($row['cname'])?>"><?=$esc($row['cname'])?></a><?php endforeach; ?></div></section><section class="youdu-section"><div class="youdu-section-head"><h3>章节目录（第<?=$pid_safe?>页）</h3><span class="meta"><?php if($chapters_safe > 0): ?>共<?=$chapters_safe?>章<?php endif; ?></span></div><div class="youdu-chapter-grid youdu-chapter-grid--compact"><?php foreach($list_arr_safe as $row): ?><a href="<?=$esc($row['cid_url'])?>" title="<?=$esc($row['cname'])?>"><?=$esc($row['cname'])?></a><?php endforeach; ?></div><?php if($htmltitle_safe !== ''): ?><div class="youdu-page-box"><?=$htmltitle_safe?></div><?php endif; ?></section></div>
